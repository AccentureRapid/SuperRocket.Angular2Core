...
{
    public abstract class Angular2CoreTestBase : AbpIntegratedTestBase<Angular2CoreTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected Angular2CoreTestBase()
        {
        ...
        protected void UsingDbContext(Action<Angular2CoreDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Angular2CoreDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Angular2CoreDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Angular2CoreDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Angular2CoreDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Angular2CoreDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Angular2CoreDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Angular2CoreDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Angular2CoreDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Angular2CoreDbContext, Task<T>> func)
        {
            T result;
